...
SENSORS: tuple[Desc, ...] = (
    ...
    Desc(
        ...
        icon="mdi:flash",
        data_key=ATTR_LAST_ACTION,
        entity_category=DIAG_ENTITY_CATEGORY,
    ),
    Desc(
        key="last_action_at",
        name="Время последнего действия",
        icon="mdi:clock-outline",
        data_key=ATTR_LAST_ACTION_AT,
        device_class="timestamp",
        entity_category=DIAG_ENTITY_CATEGORY,
    ),
    Desc(
        key="last_reason",
        name="Причина последнего решения",
        icon="mdi:comment-question",
        data_key=ATTR_LAST_REASON,
        entity_category=DIAG_ENTITY_CATEGORY,
    ),
    Desc(
        key="last_lux",
    ...
    Desc(
        key="next_off_at",
        name="Запланированное выключение",
        icon="mdi:timer-stop-outline",
        data_key=ATTR_NEXT_OFF_AT,
        device_class="timestamp",
        entity_category=DIAG_ENTITY_CATEGORY,
    ),
)
...
class DiagSensor(SensorEntity):
    ...
    async def async_added_to_hass(self) -> None:
        ...
        self._unsub = async_register_entity_update_listener(
            self.hass, self.entry.entry_id, self.async_write_ha_state
        )
    ...
    def native_value(self):
        ...
        if self.entity_description.device_class == "timestamp" and isinstance(
            value, str
        ):

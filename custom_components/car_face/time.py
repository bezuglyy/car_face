...
from .entity_base import CarFaceRestoreBase
...
async def async_setup_entry(hass, entry: ConfigEntry, async_add_entities) -> None:
    async_add_entities(
        [
            CarFaceTime(hass, entry, CTRL_START_TIME, "Время начала", _parse(DEFAULT_START_TIME, dt_time(0, 0, 0)), "mdi:clock-start"),
            CarFaceTime(hass, entry, CTRL_END_TIME, "Время окончания", _parse(DEFAULT_END_TIME, dt_time(23, 59, 59)), "mdi:clock-end"),
        ]
    )


class CarFaceTime(CarFaceRestoreBase, TimeEntity):
